...
| R-trust-official-docs | External systems' officially documented behavior is trusted as published; empirical validation is required only where the documentation is vague or insufficient for the specific guarantee being relied upon (owner ruling 2026-09-01; applied to A1 — DuckDB WAL/checkpointing — consistent with A2/A3). The vague-docs exception is invoked explicitly, in the relying change's review, naming what the docs fail to pin down. | Review practice, recorded in AGENTS.md; the A1–A4 assumption table in specs/README.md carries the applications | s§9.6 procedure (below) |
