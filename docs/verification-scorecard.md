...
| instr-gate (#46) / smoke (#47) | instr-gate: 0 regressions caught yet, teeth present via 2 `iai-callgrind` benches (PR #160). smoke: 1 self-catch — cold release-compile time (886.9s) counted as measured test runtime against a 42s ceiling, fixed by adding an untimed `--no-run` pre-build pass (PR #164, commit `522d9521`). | In-CI, per-PR. | keep |
