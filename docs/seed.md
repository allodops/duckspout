...
| D-11 | Spike | `spike/` dir at root, excluded from the workspace (`exclude = ["spike"]` ⁂). Exempt from all gates ⁂ — being workspace-excluded, no cargo gate reaches it; the repo-wide rules that still bound it were the `banned-file` globs (no `*.sh`) and the anyone→spike forbidden edges (s§7); spike/README asked for fmt-clean code as a courtesy, not a gate. Covered the full §12.1 thread including Airport-served query and the hot∪cold union ⁂. **Deleted at v0.1** (issue #122, once #35/#36 superseded it as executable reference): `spike/`, the workspace `exclude`, and the `spike`-pattern `banned-source` rule are all gone; lessons survive as ADRs and issues. |
